...
## 7.1. Implementation trong repo

Mã nguồn nằm trong `src/ecommerce_multiagent/`. Chạy toàn bộ 50 case:
...
Mỗi run xóa các `output/EC_*.json` cũ và truncate `logging/trace.jsonl`, rồi chỉ ghi output đã qua verifier. Kết quả hợp lệ phải có đúng 50 JSON và lệnh chạy trả exit code 0.
